...
import com.marcelo.springboot.app.util.paginator.PageRender;
...
public class ClienteController {
	...
	public String guardar(@Valid Cliente cliente, BindingResult result, Model model,
		...
		if (!foto.isEmpty()) {
			...
			log.info("Absolute Path: " + rootAbsolutPath);
			try {
	...
	public String eliminar(@PathVariable(value = "id") Long id, RedirectAttributes flash) {
		...
		if (id > 0) {
			...
			Path rootPath = Paths.get(UPLOADS_FOLDER).resolve(cliente.get().getFoto()).toAbsolutePath();
			File archivo = rootPath.toFile();

			if (archivo.exists() && archivo.canRead()) {
				if (archivo.delete()) {
					flash.addFlashAttribute("info", "Foto " + cliente.get().getFoto() + " eliminada con exito");
				}
			}
		}
		return "redirect:/listar";
